...
export const matches =
  ...
  (v) => {
    ...
    if (typeof v == "number") return v.toString().toLowerCase().includes(x);
      ...
      return v.some((y) => y.toLowerCase().includes(x));
    else return false;
  };
...
export const parse_ast = (str) => {
  ...
  const redu = (str, rex = false) => {
    ...
    str = str.substring(r ? 1 : 0, r ? str.length - 1 : str.length);
...
export const ast_match =
  ...
  (ast, vals, obj) => {
    ...
    switch (ast.op) {
      case "&":
        return (
          ast_match(ast.left, vals, obj) && ast_match(ast.right, vals, obj)
        );
      case "|":
        return (
          ast_match(ast.left, vals, obj) || ast_match(ast.right, vals, obj)
      ...
      case ":": {
        if (typeof ast.left == "string")
          return ast_match(ast.right, [obj[ast.left]], obj);
        switch (ast.left.op) {
          case "&":
            return (
              ast_match(
                { op: ":", left: ast.left.left, right: ast.right },
                vals,
                obj,
              ) &&
              ast_match(
                { op: ":", left: ast.left.right, right: ast.right },
                vals,
                obj,
              )
            );

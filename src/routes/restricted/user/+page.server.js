...
export const actions = {
  ...
  pwd_chg: async ({ request, locals }) => {
    ...
    try {
      await prisma.user.update({
        where: { id: user_id },
        data: {
          password: hash_password(locals.user.login, new_pwd.toString()),
        },
      });
    } catch (error) {
      console.error(error);
  ...
  update_data: async ({ request, locals }) => {
    ...
    if (keys_to_update.some((k) => !data.has(k) && !bool_entries.includes(k)))
      return { update_failure: "Invalid form data submitted!" };

    let prisma_data = Object();
    prisma_data["sleeping"] = false;
    for (let k of keys_to_update)
      prisma_data[k] = data.get(k)?.toString().trim();
    /// @ts-ignore
    for (let k of list_entries)
      prisma_data[k] = prisma_data[k].split(",").map((x) => x.trim());
  ...
  create_account: async ({ locals, request }) => {
    ...
    if (!locals.user.admin)
      return { creation_failure: "user does not have permissions" };
    ...
    const password_hash = hash_password(
      login,
  ...
  mass_create_accounts: async ({ locals, request }) => {
    ...
    const prenom_nom = emails_s
      ...
      .map(([pre, nom]) => [
        capFirst(pre.replaceAll("_", " ").trim()),
        capFirst(nom.replaceAll("_", " ").trim()),
      ]);

    const unique_logins = await Promise.all(
      prenom_nom
        .map(([prenom, nom]) =>
          (nom + " " + prenom).replaceAll(" ", "-").toLowerCase(),
        )
        .map(async (login_o) => {
          // make it unique
          const clashes = (
    ...
    const passwords = Array.from(unique_logins, (_login) =>
      generate_password(),
    );
    /** @type {string[]} */
    ...
    try {
      await prisma.user.createMany({
        data: creation_data.map(
          ([prenom, nom, login, pwdhash, _clearpwd, email]) => ({
            prenom,
            nom,
            login,
            grad_year,
